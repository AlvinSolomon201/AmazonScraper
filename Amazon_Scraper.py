...
def sendAlert(message):
    ...
    to = ['XXXXXXX']
    subject = 'Apple iPhone 12 Price Alert'
...
def search_product_list(interval_count = 1, interval_hours = 6):
    ...
    """
    prod_tracker = pd.read_csv('trackers/TRACKER_PRODUCTS.csv', usecols=['url','code','buy_below'])
    ...
    while interval < interval_count:
        ...
        for x, url in enumerate(prod_tracker_URLS):
            ...
            try:
                price = float(soup.find("span",class_="a-offscreen").get_text().strip().replace('₹','').replace(',',''))
            except:
                price = ''
            ...
            try:
                ...
                if price < prod_tracker.buy_below[x]:
                    ...
                    AlertMessage = f'Price has gone below {prod_tracker.buy_below[x]}!!! Buy Now'
                    sendAlert(AlertMessage)
                else:
                    print('price is high')
                    AlertMessage = f'Price is above {prod_tracker.buy_below[x]}... Dont Buy.'
                    sendAlert(AlertMessage)
            except:
